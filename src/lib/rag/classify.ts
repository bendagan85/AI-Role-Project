...
export async function classifyCoachRaw(
  ...
  const { object } = await generateObject({
    ...
    prompt: [
      'You are placing a coach into a fitness-platform directory.',
      'The platform is for TRAINING and NUTRITION coaches.',
      '',
      'Pick the single best category id:',
      '- training: fitness, strength, conditioning, exercise, sports, technique,',
      '  movement — and any generic "coach"/"trainer"/"מאמן" with no other',
      '  domain stated (default the generic/ambiguous case here).',
      '- nutrition: diet, performance nutrition, meal planning, supplements, dietitian.',
      '- other: ONLY when the coach is CLEARLY about an unrelated domain —',
      '  e.g. gaming/Minecraft, software, business, finance, languages, music,',
      '  life/mindset, entertainment.',
      '',
      'Be lenient. The persona may be terse or in any language (Hebrew "מאמן"',
      '= trainer → training). If it is generic, brief, or ambiguous, choose',
      'training or nutrition — NOT other. Choose other only when an unrelated',
      'domain is explicit and unmistakable.',
      '',
      ...(name ? [`Name: ${name}`, ''] : []),
      `Persona: ${persona}`,
      '',
